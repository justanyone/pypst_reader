...
### P12-FUZZ
status: ✅ 2026-09-16 — tests only. `tests/corrupt.py` gained the mutation generator: `Mutation(name, data, expect, must_raise)`, `mutations(store_bytes, seed=)`, eight families in `FAMILIES` (`truncations`, `bit_flips`, `field_lies`, `pointer_cycles`, `depth_bombs`, `future_versions`, `zero_files`, `magic_only`), field offsets derived from the modules' struct formats (`struct_fields`, `header_fields`, `page_fields`), plus `btree_chain`, `append_pages`, `replace_page`, `tree_pages`, `pages_read`, `root_refs`, `leaf_page`, `mutation(name=)` for P24. Per base, one seed: pstd-inline-cid 398 mutations (61/72/230/6/3/17/6/3 per family), Empty 472 (62/96/279/6/3/17/6/3); counts pinned in `tests/test_corrupt_generator.py`. `tests/corruption_harness.py` runs `read_header`, both B-tree walks + `find`, the density list (and the trees the mutated header names) under a thread watchdog (5 s; a hang leaks the worker, documented) and classifies LEAK / TYPE / SILENT / HANG, baselining each entry point against the unmutated store. `tests/test_corruption.py`: 16 family×base sweeps + summary + named cases (`!BDN` then nothing; ROOT byte index past EOF for NBT/BBT via `read_header`) + 7 `importorskip` stubs for P03 (lying cb, XXBLOCK 10 000 deep, 4 GB lcbTotal, subnode cycle, leaf data block past EOF) and P04 (BTH cycle, HID past block). `scripts/fuzz_sweep.py` (stdlib): 8 Unicode fixtures × 5 seeds × 8 families = **18 527 mutations, 0 leaks, 0 hangs, 0 wrong types, 17.4 s**; wired as a `slow` test with 3 seeds. Suite cost: ~1.4 s added to `uv run pytest` (67 new tests, 7 skipped stubs). 11 deliberate breakages each confirmed red with bytecode disabled (depth check disabled → SILENT; PstError re-raised as IndexError → LEAK; family removed → pinned counts and summary; flips not resealed; truncate off by one; must_raise dropped; short header / short page as PstLimitError → named cases; future version dropped; unseeded rng). **No leak found in the landed code** (`header`, `root`, `page`, `btree`, `limits`), so no xfail was pinned. Cases already in test_header.py/test_btree.py (bad CRC, wVer future, NBT root → itself, child past EOF) are not duplicated; the families cover them with `expect` pinned to the exact type.
upstream: none
