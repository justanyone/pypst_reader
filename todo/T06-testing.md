...
### P24-CONTRACT
status: ✅ 2026-09-16 — tests only (plus `pypst.__all__`). `src/pypst/__init__.py` now exports, sorted: `DEFAULT_LIMITS`, `Header`, `Limits`, `PstError`, `PstFormatError`, `PstLimitError`, `PstNotFoundError`, `PstUnsupportedError`, `__version__`, `read_header` (`open`/`Store` deliberately absent until P07). `tests/contract.py` **discovers** the entry points — `pkgutil` over the package, each module's `__all__` else its public top-level callables, every public classmethod and method along the pypst MRO — and classifies each by its parameters (bytes / file / reader / path / wire / method); today **115 public callables** across 16 modules: 57 with an `ADAPTERS` builder (a `Store` hands each the bytes, the file, the parsed header, the two B-trees, a `BlockReader`, the NBT/BBT entries, nine pages, a temp path — `read_header` ← `BytesIO`; `NodeBTree` ← `(f, header.root.node_btree, limits)`; `BlockReader.node_data` ← every NBT entry; `read_block` ← every BBT entry, plus the wrong `is_internal` flag on four; `decode`/`from_wire` ← every type code + edges + a seeded sample × slices; every registered `debug.DUMPERS` function automatically, `nid` extras via `EXTRA_ARGS`), 21 named in `NOT_STORE_INPUT` with a reason (check helpers, `pack`, `from_parts`, signature arithmetic, `verify_*` reached through `read_block`), 37 excluded by rule (exception types, enums, dataclass constructors). `uncovered()`/`stale()` make the contract loud: `test_every_public_callable_has_an_adapter_or_a_reason` lists any new public callable until its row places it. `judge` → OK / PST_ERROR(type) / LEAK(type, innermost frame) / HANG under P12's `Watchdog` (one worker submission per store, the worker naming its current entry point so a hang is attributed); `sweep(data, limits=, thorough=)`; `problems()` = LEAK + HANG only. **Measured:** corpus 10 fixtures (8 Unicode, 2 ANSI) × 4 limit configurations (default; every ceiling 1 with `max_file_size` default; `max_file_size`=1 alone; every ceiling `sys.maxsize`) — 725–1225 calls per fixture at full breadth, **0 leaks, 0 hangs**; ANSI: `read_header`/`Header.parse` → `PstUnsupportedError` and exactly the 16 reader entry points SKIP; Unicode: every entry point reached (no SKIP); ceilings at 1 trip `PstLimitError` in the readers on all 8 Unicode stores and nothing but `PstLimitError`/`PstFormatError`; ceilings at max reproduce the default outcomes reader for reader on all 10 (a difference would name the ceiling via single-ceiling bisection). Corruption: non-slow 398 mutations (pstd-inline-cid, seed 0, all 8 families) × 49 store-dependent adapters (~540 calls each; the 8 store-free wire adapters run only in the fixture lanes) — **0 leaks, 0 hangs, ~4.5 s**; slow: 3 seeds × 8 Unicode bases = **11 113 mutations × 57 adapters, 0 leaks, 0 hangs, 139 s**. CLI (slow): `python -m pypst.debug <4 dumpers> <10 fixtures + 5 mutations>` = 60 subprocesses, one at a time: exit 0 clean or exit 1 with `Error:`, no traceback. 45 tests (43 non-slow in ~4.7 s on a loaded box, 2 slow). 7 deliberate breakages confirmed red with bytecode disabled (struct.error re-raised from `ids._unpack` → LEAK in both sweeps; an adapter deleted → uncovered; `__all__` unsorted; ceilings left at default → "no reader tripped"; `MAX_ITEMS = 20` → not-tight naming `max_items` on javalibpst; a dumper raising `ValueError` → CLI traceback; an unbounded `pages()` walk → HANG after 5 s — that runaway thread then grew the stack until the kernel OOM-killed the process, P12's documented watchdog cost, so the judge's HANG test uses a bounded sleeper). **No leak found in the landed code; no xfail pinned.** One observation, not a leak: `encode.decode_block` raises `ValueError` for an unknown `CryptMethod`, but the parameter is the enum and `header` refuses an unknown method byte first, so it is unreachable from a file. docs/TEST-PLAN.md § T5 says how a new layer joins; docs/INTERFACES.md § top level records `__all__`.
upstream: none
oracle:   none
