...
them. Every row here is a leaf — none touches `src/pypstreader/ndb/`, `ltp/` or
...
status: ✅ 2026-09-15 — `parse_read_header` parses 9/9 read_header goldens (Empty + 8 corpus; 2 ANSI
        ...
        `python -m pypstreader.debug --list` runs with an empty registry
upstream: none
oracle:   `tests/golden/`
...
1. `src/pypstreader/debug.py` — `python -m pypstreader.debug <layer> <file>`, a
...
- Exits explained: upstream reads BOTH ANSI stores (they did not "error at open"; pypstreader still refuses
...
### P24-CONTRACT
status: ✅ 2026-09-16 — tests only (plus `pypstreader.__all__`). `src/pypstreader/__init__.py` now exports, sorted: `DEFAULT_LIMITS`, `Header`, `Limits`, `PstError`, `PstFormatError`, `PstLimitError`, `PstNotFoundError`, `PstUnsupportedError`, `__version__`, `read_header` (`open`/`Store` deliberately absent until P07). `tests/contract.py` **discovers** the entry points — `pkgutil` over the package, each module's `__all__` else its public top-level callables, every public classmethod and method along the pypstreader MRO — and classifies each by its parameters (bytes / file / reader / path / wire / method); today **176 public callables** across 18 modules: 101 with an `ADAPTERS` builder (a `Store` hands each the bytes, the file, the parsed header, the two B-trees, a `BlockReader`, the NBT/BBT entries, nine pages, every node opened as a heap, the BTH over each, and the property or table context of each heap that is one, a temp path — `read_header` ← `BytesIO`; `NodeBTree` ← `(f, header.root.node_btree, limits)`; `BlockReader.node_data` ← every NBT entry; `read_block` ← every BBT entry, plus the wrong `is_internal` flag on four; `HeapNode.from_node` ← every NBT entry and the first sub-node leaves; `HeapNode.get` ← the user root, item 1 of the first and of the last block, the null HID, one past `cAlloc`, one past the last block; `HeapTree` ← each heap's user root and a table context's `hidRowIndex`; `PropertyContext.read` ← every record of every PC plus four forged ones; `format_property_value`/`property_lines` ← every value the corpus's PCs decode to; `TableContext.read_cell` ← every present cell of the rows kept plus four forged records; `check_existence_bitmap` ← every column's `iBit` against a full and an empty bitmap of the schema's own width; `decode`/`from_wire` ← every type code + edges + a seeded sample × slices; every registered `debug.DUMPERS` function automatically, `nid` extras via `EXTRA_ARGS`), 26 named in `NOT_STORE_INPUT` with a reason (check helpers, `pack`, `from_parts`, `from_debug_name`, signature arithmetic, `HeapPageMap.size`'s caller-checked index, `verify_*` reached through `read_block`), 49 excluded by rule (exception types, enums, dataclass constructors). `uncovered()`/`stale()` make the contract loud: `test_every_public_callable_has_an_adapter_or_a_reason` lists any new public callable until its row places it. `judge` → OK / PST_ERROR(type) / LEAK(type, innermost frame) / HANG under P12's `Watchdog` (one worker submission per store, the worker naming its current entry point so a hang is attributed); `sweep(data, limits=, thorough=)`; `problems()` = LEAK + HANG only. **Measured:** corpus 10 fixtures (8 Unicode, 2 ANSI) × 4 limit configurations (default; every ceiling 1 with `max_file_size` default; `max_file_size`=1 alone; every ceiling `sys.maxsize`) — 1031–14 233 calls per fixture at full breadth, **0 leaks, 0 hangs**; ANSI: `read_header`/`Header.parse` → `PstUnsupportedError` and exactly the 40 reader entry points SKIP; Unicode: every entry point reached (no SKIP); ceilings at 1 trip `PstLimitError` in the readers on all 8 Unicode stores and nothing but `PstLimitError`/`PstFormatError`; ceilings at max reproduce the default outcomes reader for reader on all 10 (a difference would name the ceiling via single-ceiling bisection). Corruption: non-slow 458 mutations (pstd-inline-cid, seed 0, all 11 families) × 91 store-dependent adapters (~330–1050 calls each; the 10 store-free wire adapters run only in the fixture lanes) — **0 leaks, 0 hangs, ~9 s**; slow: 3 seeds × 8 Unicode bases = **12 571 mutations × 101 adapters, 0 leaks, 0 hangs, 284 s**. CLI (slow): `python -m pypstreader.debug <8 dumpers> <10 fixtures + 5 mutations>` = 120 subprocesses, one at a time: exit 0 clean or exit 1 with `Error:`, no traceback. 48 tests (46 non-slow in ~13 s on a loaded box, 2 slow in ~296 s). 7 deliberate breakages confirmed red with bytecode disabled (struct.error re-raised from `ids._unpack` → LEAK in both sweeps; an adapter deleted → uncovered; `__all__` unsorted; ceilings left at default → "no reader tripped"; `MAX_ITEMS = 20` → not-tight naming `max_items` on javalibpst; a dumper raising `ValueError` → CLI traceback; an unbounded `pages()` walk → HANG after 5 s — that runaway thread then grew the stack until the kernel OOM-killed the process, P12's documented watchdog cost, so the judge's HANG test uses a bounded sleeper). **No leak found in the landed code; no xfail pinned.** One observation, not a leak: `encode.decode_block` raises `ValueError` for an unknown `CryptMethod`, but the parameter is the enum and `header` refuses an unknown method byte first, so it is unreachable from a file. **P04 placed (2026-09-16):** the 21 heap/BTH entry points the rebase brought in are adapted, not excused — `HeapNode` is now in `READER_TYPES`, so `HeapTree` and its walks classify as readers and SKIP on an ANSI store with the rest; `Store` gained `heaps` (every node opened with `HeapNode.from_node`, plus up to eight sub-node leaves' own heaps, a node that is not a heap simply left out), `trees` (the BTH at each heap's user root and at a TC's TCINFO `hidRowIndex`) and `heap_buffers` (real block-0 and user-root-item bytes, at start/straddle/end/past-end offsets, for `HeapId`/`HeapNodeId`/`HNHDR`/`BTHHEADER` `unpack_from` — swallowing the refusal, because a `bytes` entry point must still be reached on an ANSI store). Only `HeapId.from_parts`/`pack`, `HeapNodeId.pack` and `HeapPageMap.size` are in `NOT_STORE_INPUT`; `HeapNodeType.from_wire` is adapted over the edge bytes and all nine of [MS-PST] 2.3.1.2. `read_data_blocks`/`node_data_blocks` are adapted as `read_data`/`node_data` are. Over the corpus that is 19–115 heaps and 19–111 BTHs per Unicode store: **0 leaks, 0 hangs, no xfail pinned** — a wrong-signature node, a non-heap node, an HID past `cAlloc` or past the last block and an absent sub-node all come out as `PstFormatError`/`PstNotFoundError`. **P05 placed (2026-09-16):** the nine property-context entry points are adapted — `Store` gained `contexts` (a `PropertyContext` over every heap that is one; a TC or a bare BTH refuses, which is the contract and is judged at the constructor's own adapter), `pc_records`, `pc_values` (every record decoded, for the two formatters `dump_pc` prints with — swallowing the refusal, as they are `wire` entry points that must be reached on an ANSI store too) and `bth_leaves` (real `(key, value)` pairs for `PropertyRecord.unpack`, short, doubled, and one record per wire type code). `read` takes every record of every PC plus four forged ones — an HNID naming a sub-node no tree holds, type bits on a `PtypGuid`, a null HNID, an HID past `cAlloc` — and the first PC's records again through a code page that does not exist (a `PstError`, never the codecs' `LookupError`); `get` takes a held id, a null-HNID id, an absent id and 0. `PropType.from_debug_name` is excluded as `NodeIdType.from_debug_name` is. Over the corpus that is 7–38 contexts and 78–774 records per Unicode store: **0 leaks, 0 hangs, no xfail pinned.** **P06 placed (2026-09-16):** the 15 table-context entry points are adapted — `Store` gained `tables` (a `TableContext` over every heap that is one; a PC or a bare BTH refuses, which is the contract), `table_rows` (the first eight rows of each, decoded; `rows()` still walks every one), `table_cells` (every cell of those rows, for `cell_lines`/`format_cell_record`) and `tcinfo_buffers` (real TCINFO items for `TableContextInfo.unpack` and `ColumnDescriptor.unpack_from`, at every edge and with every wire type code in a TCOLDESC). `row` takes 0, the last row, one past it and a negative; `find_row` a held id, an absent one and 0; `read_cell` every present cell plus an HID past `cAlloc`, a sub-node no tree holds, a null HNID and an inline cell too short for its type; `get_hnid_blocks` shares `get_hnid`'s builder. Over the corpus that is 0–73 tables per Unicode store — pstd-inline-cid's only TC is refused by design at the existence-bitmap check and surfaces as `PstFormatError`, javalibpst's IPM table is the sub-node-matrix witness: **0 leaks, 0 hangs, no xfail pinned.** One observation, not a leak, and the reason no negative column index is fed: `check_existence_bitmap(-1, b"")` would raise `IndexError`, but `iBit` is a `u8` out of a TCOLDESC, so a negative index is a caller's bug and not a file's — the same reason `HeapPageMap.size` is excluded. docs/TEST-PLAN.md § T5 says how a new layer joins; docs/INTERFACES.md § top level records `__all__`.
upstream: none
oracle:   none
blocked on: P12 (the corruption corpus), P01 (the first entry point)

`tests/test_contract.py`: for every public callable in `pypstreader.__all__`, for
...
`pytest-cov` in the dev group, `--cov=pypstreader --cov-report=term-missing`, and
