...
`PstUnsupportedError` naming the version and `pypstreader_nu`; 23/36/37 are
parsed; anything else is `PstFormatError`. One struct format per structure,
no variant axis, no `PstFile` trait.

**Done means:**
- `python -m pypstreader.debug header <f>` parsed by P29's `parse_read_header`
