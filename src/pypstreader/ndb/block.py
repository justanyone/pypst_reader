...
A node's bytes live in blocks. The block B-tree (`pypstreader.ndb.btree`) says
...
**Divergence: every walk is bounded — `pypstreader.limits`, wired here.**
...
from pypstreader.encode import CryptMethod, decode_block
...
from pypstreader.ndb.btree import BlockBTree
from pypstreader.ndb.header import Header
from pypstreader.ndb.ids import BlockId, BlockRef, ByteIndex, NodeId, _unpack
from pypstreader.ndb.page import BlockBTreeEntry, NodeBTreeEntry
...
class BlockReader:
    ...
    def read_data_blocks(self, block: BlockId) -> list[bytes]:
        ...
        The per-block split is what a Heap-on-Node needs (`pypstreader.ltp.heap`):
