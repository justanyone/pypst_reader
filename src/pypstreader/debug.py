"""Per-layer dump entry point: ``python -m pypstreader.debug <layer> <file>``.
...
from pypstreader.eml import eml_bytes, export_folder, folder_paths
...
from pypstreader.messaging.named_prop import PS_MAPI, PS_PUBLIC_STRINGS
from pypstreader.messaging.store import Store
from pypstreader.ndb.block import BlockReader, DataBlock, SubNodeLeafBlock
from pypstreader.ndb.btree import BlockBTree, NodeBTree, read_density_list
from pypstreader.ndb.header import read_header
from pypstreader.ndb.ids import NID_ROOT_FOLDER, BlockId, NodeId, NodeIdType
...
def dump_store(path: Path) -> None:
    ...
    **The store opens where the example fails.** `pypstreader.messaging.store`
...
def folder_table(folder: Folder, node_type: NodeIdType) -> TableContext | None:
    ...
    (`pypstreader.messaging.folder`'s module docstring says why), which is the
...
def dump_folders(path: Path) -> None:
    ...
    exception is — and `python -m pypstreader.debug folders` still exits 0, as the
...
def upstream_records(pc: PropertyContext) -> dict[int, PropertyRecord]:
    ...
    nowhere else — `pypstreader.messaging.attachment`'s module docstring
...
def _open_message_as_upstream(store: Store, node: NodeId) -> Message:
    ...
    divergence `pypstreader.messaging.message`'s docstring records.
    """
...
def dump_messages(path: Path) -> None:
    ...
    attachments that `pypstreader.messaging.attachment` can and upstream cannot
...
def dump_eml(path: Path, nid: str) -> None:
    ...
    The bytes `pypstreader.eml.eml_bytes` produces, which are pure ASCII with CRLF
...
def dump_export(path: Path, dest: str, *, as_eml: bool = False) -> None:
    ...
    refuses (`pypstreader.eml.export_folder`'s `strict=False`), so one unreadable
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m pypstreader.debug",
