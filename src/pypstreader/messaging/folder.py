...
- **A table node that exists but is not a readable table context is a
  ...
  **the node is not in the node B-tree** (`PstNotFoundError` from the NBT)
  ...
  `pypstreader.debug`'s `folders` dumper reproduces upstream's `.ok()?` itself, so
...
- **`walk()` is iterative, bounded, and cycle-guarded.** Upstream has no
  ...
  which is what `pypstreader.limits` prescribes for a cycle) and a depth ceiling,
...
so the two agree, and `python -m pypstreader.debug folders` reproduces the golden's
...
from pypstreader.limits import VisitedSet, check_count, check_depth
