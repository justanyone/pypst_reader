...
in `pypstreader.ltp.prop_type` (`count > max_items` raises), and the tests pin both
...
from pypstreader.errors import PstLimitError
...
# [MS-PST] 2.2.2.1 NID: `nidIndex` is 27 bits (`pypstreader.ndb.ids.MAX_NODE_INDEX`
...
# Elements in one multi-valued property (`pypstreader.ltp.prop_type.decode`'s
