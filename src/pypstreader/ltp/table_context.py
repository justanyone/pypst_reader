...
Ported from: crates/pst/src/ltp/table_context.rs (the Unicode arm of
             ...
             P22's `pypstreader.ltp.prop_type`)
...
(`pypstreader.ltp.heap`) whose client signature is `bTypeTC` (0x7C), whose user
...
(`pypstreader.ltp.tree`) keyed by the 4-byte row id whose 4-byte value is the
...
from pypstreader.errors import PstFormatError, PstNotFoundError, PstUnsupportedError
from pypstreader.limits import Limits, check_allocation, check_count
...
from pypstreader.ndb.ids import NodeId, _unpack
