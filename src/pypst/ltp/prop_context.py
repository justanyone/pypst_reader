...
Ported from: crates/pst/src/ltp/prop_context.rs (the record types, `PropertyContextInner::properties`
             and `read_property`; the value decoders are P22's `pypst.ltp.prop_type`)
...
in: a heap (`pypst.ltp.heap`) whose client signature is `bTypePC` (0xBC)
and whose user root is a BTH (`pypst.ltp.tree`) keyed by the 2-byte
...
from pypst.limits import Limits, check_count
