...
(`pypst.ndb.root`) — the pages the node and block B-trees start from. It is
...
what `PstUnsupportedError` means (`pypst.errors`); parsing them with the
...
`pypst.encode`, where the decoders are; it is re-exported here.
...
from pypst.ndb.ids import BlockId, PageId
from pypst.ndb.root import Root
...
def _check_version(raw: int) -> Version:
    ...
    if version.is_ansi:
        raise PstUnsupportedError(
            f"ANSI (pre-2003) store, wVer={raw}; pypst reads Unicode stores only — see pypst_reader_nu"
        )
    if version is not Version.UNICODE:
        raise PstUnsupportedError(
            f"wVer={raw} (4 KB-page Unicode store) is not supported; pypst reads wVer 23 stores only"
        )
    return version
