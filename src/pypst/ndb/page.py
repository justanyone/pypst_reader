...
file: a page arrives as 512 bytes that `pypst.ndb.btree` fetched, and it is
refused or turned into frozen values here. The walks that follow page
references are in `pypst.ndb.btree`.
...
from pypst.ndb.ids import BlockId, BlockRef, ByteIndex, NodeId, PageId, PageRef, _unpack
...
# (`InvalidBTreePageLevel`); `pypst.limits.MAX_BTREE_DEPTH` is chosen to
