...
`PidTagRtfCompressed` (0x1009) is LZFu-compressed RTF that `pypst.rtf`
(P21) expands.
...
from pypst.limits import check_count, check_depth
from pypst.ltp.prop_context import PropertyContext, PropertyRecord
from pypst.ltp.prop_type import PropType, PropValue, filetime_to_datetime
from pypst.ltp.table_context import TableContext, TableRow
...
class Message:
    ...
    *embedded* message is not in the node B-tree at all — it is a sub-node of
    ...
    def body_rtf_decompressed(self) -> bytes | None:
        """`body_rtf` expanded by `pypst.rtf` — the `{\\rtf1` document — or `None` when there is no RTF body.
        ...
        as a single NUL). `pypst.rtf.decompress_rtf` deliberately does not
        ...
        A body that is not LZFu is `PstFormatError` from `pypst.rtf`.
        """
        data = self.body_rtf
        if data is None:
            return None
