...
3. **Every header this module added is named in `X-Pypst-Synthesized`**, a
...
`X-Pypst-Synthesized`. When it does not, the id is built from the store's
record key and the message's node id under the reserved domain
`pypst.invalid` (RFC 2606), which makes an invented id recognisable by
inspection as well as by the marker: `<pypst-<record-key>-<nid>@pypst.invalid>`.
...
`X-Pypst-Body: rtf-only` (`X-Pypst-Body: none` when the message has no body
...
`X-Pypst-Attachment-Skipped: <method> <filename>` — rather than dropped
...
- **Part boundaries are deterministic.** `email` picks them from
  ...
  `----=_pypst.<nid>.<n>`, extended with a counter if that string occurs in
...
from pypst.limits import Limits, VisitedSet, check_allocation, check_depth
...
if TYPE_CHECKING:  # pragma: no cover - import cycle, types only
    ...
    from pypst.messaging.message import Attachment, Message
...
SYNTHESIZED_HEADER = "X-Pypst-Synthesized"
BODY_HEADER = "X-Pypst-Body"
SKIPPED_HEADER = "X-Pypst-Attachment-Skipped"
...
SYNTHETIC_ID_DOMAIN = "pypst.invalid"
...
def _recipient_headers(message: Message) -> dict[str, str]:
    ...
    """
    from pypst.messaging.message import RecipientType
...
def _synthetic_message_id(message: Message, prefix: str | None) -> str:
    ...
    """
    local = f"pypst-{_record_key(message)}-{message.node.raw:08x}" if prefix is None else f"{prefix}.{message.node.raw:08x}"
...
def _message_id_local_part(value: str) -> str:
    ...
    return inner.split("@", 1)[0] or "pypst"
...
def to_eml(
    ...
    *,
    ...
    module docstring).
    """
    from pypst.messaging.message import Message as _Message
...
def _add_attachments(msg: EmailMessage, message: Message, limits: Limits, *, depth: int, prefix: str | None) -> None:
    """Every attachment of `message`, as a part or as an `X-Pypst-Attachment-Skipped` header."""
