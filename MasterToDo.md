...
| P10-EML | ✅ 2026-09-16 | `eml.py` (`to_eml`/`eml_bytes`/`write_eml`, not a port) + `mbox.py` (`export_mbox`, one RFC 4155 mbox per folder via stdlib `mailbox`, `folders.txt` NID→name index), `debug eml`, `debug export` (mbox default, `--eml`). Header policy from P09's measurement: transport headers passed through in source order minus `Content-*`, the rest synthesised from MAPI and **named in `X-Pypst-Synthesized`**; `Message-ID` from 0x1035 else deterministic `@pypst.invalid`. Round trip: synth-basics' Sent message equals its source `.eml` through both `to_eml` and mbox. 12/12 openable corpus messages export (ASCII+CRLF, deterministic, re-parse identical); private 2/2 counts only. 116 cases, 31/31 mutants red, harness green, tier T11 added. Not done: DSN/TNEF/iCalendar rendering, Maildir. |
