...
| P09-MESSAGE | 5 | ⏳ in flight — agent/p09-message 2026-09-16 | `messaging/message.py` + `attachment.py` — properties, bodies (plain/HTML/RTF via P21), recipients, attachments, embedded messages (`pstsdk-submessage.pst`). | [`todo/T03-messaging.md`](todo/T03-messaging.md#p09-message) |
| P06b-EMPTY-TC | 2 | ⏳ in flight — agent/p06b-empty-tc 2026-09-16 | `synth-basics.pst`'s root folder writes an **empty** associated-contents table with `rgib[TCI_4b] = 4`; P06 refuses it at TCINFO parse (its `≥ 8` divergence) where upstream never reads a row. Accept a TCINFO whose row width cannot hold a row header **only when the row matrix is empty**, keep the refusal for a non-empty one, and take P08's folder dump to 8/8 byte-identical. | [`todo/T02-ltp.md`](todo/T02-ltp.md#p06b-empty-tc) |
