...
| P16-PUBLISH | 6 | ✗ blocked on P10 (user's call 2026-09-16: do it) | **Rename to `pypstreader` and publish.** `pypst` is taken on PyPI; `pypstreader` and `pstreader` are free (checked 2026-09-16). Package `src/pypst` → `src/pypstreader`; `from pypstreader import pypstreader` gives the CLI module; console script `pypstreader <file.pst>` emits an mbox by default with `--format eml`, `--output`, `--folder`, limit flags; every doc, script, test, golden parser and lint follows; `pstreader` reserved as a thin alias distribution depending on `pypstreader`. Publish with the user's token via twine from `~/.pypirc` (never in the repo or the chat). GitHub repo and local directory renames are the user's, with directions. | [`todo/T05-infra.md`](todo/T05-infra.md#p16-publish) |
